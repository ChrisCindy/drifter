...
mongoose.connect('mongodb://localhost:27017/drifter');
...
exports.getOne = function(_id, callback) {
	//通过id获取特定的漂流瓶
	bottleModel.fundById(_id, function(err, bottle) {
		if (err) {
			return callback({
				code: 0,
				msg: "读取漂流瓶失败。。。"
...
exports.reply = function(_id, reply, callback) {
	...
	bottleModel.findById(_id, function(err, _bottle) {
		...
		//更新数据库中该漂流瓶信息
		newBottle.findByIdAndUpdate(_id, newBottle, function(err, bottle) {
			if (err) {
				return callback({
					code: 0,
					msg: "回复漂流瓶失败。。。"
